...
| No internal issue references in clap `--help` text | `scripts/check-no-issue-refs-in-help.sh` |
...
- **CLI help text is product copy, not developer notes.** Every `///` inside a type deriving clap's `Parser`/`Args`/`Subcommand` is rendered verbatim into `--help`. It carries *only* what the flag does, its accepted values/format, its default, and when an operator would change it. No design rationale, no history ("supersedes…", "replaced the former…"), no benchmark narrative, no `[`RustItem`]` intra-doc links (they render as literal noise), and no issue numbers — the last of these is gated by `check-no-issue-refs-in-help.sh`; the rest need judgment, so they are a code-review dimension. Explain the *why* in the module `//!` header, where maintainers read it and operators don't.
