...
**Per-namespace tuning.** The break-even loop is the automatic default — no per-namespace action is needed to provision relays where they help. The namespaced [`CoxswainRelayPolicy`](../reference/relay-policy.md) CRD overlays overrides on the per-namespace dedicated relays only: force-on/off, `replicas`, `resources`, `podTemplate` scheduling, and opt-in autoscaling. The shared-pool relay is global — it has no policy and reads the `relay.*` values directly (autoscaling between `relay.replicas` and `relay.maxReplicas`).
