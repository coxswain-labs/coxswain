...
The provisioning operator reconciles off the controller's single watch fabric: rather than running its own kube-rs `Controller` and Kubernetes client, its reconcile is driven by the same unified status worker that writes Gateway API status, so a dedicated Gateway and its shared-pool siblings share one authoritative set of watches. Its reconcile resolves each Gateway's effective `CoxswainGatewayParameters` (per-field overlay: Gateway's `parametersRef` wins per-field, GatewayClass's fills the rest; `podTemplate` strategic-merges across both layers) and renders the desired `Deployment` / `Service` / `ServiceAccount`. The `podTemplate` escape hatch is merged onto the rendered Deployment with `kubectl apply` strategic-merge semantics — `containers` merges by `name`, `tolerations` by `(key, operator)`, container-level `env` by `name`, and so on — so sidecar injection and env overlays behave the way operators expect from native K8s tooling. Because the object carrying that overlay is tenant-writable while the pod is created by the cluster-privileged controller, the merged result is then filtered back into the [pod security envelope](../gateway-api/index.md#the-pod-security-envelope): the controller re-asserts its own pod and container `securityContext`, the pinned ServiceAccount, its own volumes, and the host namespace flags, allows only the volume sources the `restricted` Pod Security Standard permits, strips `hostPort`s, and hardens any container the overlay added. Overwritten fields are reported to the tenant as a `PodTemplateSanitized` Warning Event rather than silently dropped.
...
Required arg: `--discovery-bootstrap-endpoint` (`https://`) — the sole endpoint anchor. On first start the proxy bootstraps an SVID there, and the bootstrap response also carries its current best **routing-stream upstream** `(endpoint, expected_server_sa)`: the controller, or its namespace's relay if one is provisioned. The proxy opens the mTLS stream directly to that upstream. There is no `--discovery-endpoint`: the upstream is bootstrap-delivered and **runtime-directed** — the controller can repoint a proxy between itself and a relay by pushing a `PreferredUpstream` directive on the stream, which swaps only the control-stream connection. Routing tables are never cleared across reconnects, and **a relay rebalance never disrupts data-plane traffic**: listeners are held and the last-good routing snapshot is served throughout the brief control-stream reconnect. If a relay becomes unreachable, the proxy re-bootstraps to the controller (the always-up anchor).
