...
- **Sizing (HPA-style).** An [autoscaled](../reference/relay-policy.md#autoscaling) relay is sized `clamp(ceil(liveSubscribers / targetProxiesPerReplica), minReplicas, maxReplicas)`, with a relative **tolerance** deadband so small jitter doesn't churn and an asymmetric **scale-down stabilization** window (scale up promptly, scale down only on the trailing-window peak). The **capacity ratio** `targetProxiesPerReplica` (default 50) is deliberately decoupled from the break-even threshold: a relay is a fan-out cache (one upstream stream in, broadcast to many), so real per-replica capacity is O(100s), not the break-even number.
