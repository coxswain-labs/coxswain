...
**When a namespace relay appears — a control loop (HPA sizing + KEDA activation).** A relay costs the leader one upstream stream per replica and saves it the namespace's downstream streams, so it only *reduces* leader load past a break-even point. The controller re-implements the standard autoscaler loop internally — it cannot use a real Kubernetes HPA, because each relay replica opens the very upstream stream the tier conserves (a CPU HPA would scale *up* under load and regrow the leader fan-out) — driven by the namespace's **live dedicated-proxy subscriber count**:

- **On/off (KEDA-style).** The relay is provisioned once the subscriber count reaches the break-even **activation threshold** `relay.dedicated.minProxyReplicas` (default 8), and torn down after it holds **below** that threshold for the **cooldown** `relay.dedicated.cooldown` (default 300s). A namespace that genuinely drains (no dedicated Gateways left) is torn down at once; a *transient* drop to 0 live subscribers while Gateways remain — a relay restart or a control-stream reconnect — waits out the cooldown instead, so a blip never deletes a live relay. This replaces the old keep-until-fully-drained rule — a shrunken-but-nonzero namespace is now reclaimed after the cooldown, damped by time rather than a hand-picked lower watermark.
...
Namespace-relay autoscaling is **controller-driven, not an HPA** (see the sizing loop above). The relay is I/O/fan-out-bound (CPU mistracks its load) and each replica opens its own upstream stream to the leader, so the controller sizes it directly from the namespace's live subscriber count — `clamp(ceil(liveSubscribers / targetProxiesPerReplica), minReplicas, maxReplicas)`, damped by the tolerance deadband and scale-down stabilization window — with **`maxReplicas` mandatory** as the cap on that fan-out regrowth (an uncapped autoscaling stanza is ignored). Keep `maxReplicas` well below the namespace's downstream fan-out, or the relay's own upstream streams approach the count it is meant to collapse.
