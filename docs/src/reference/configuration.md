...
| `COXSWAIN_EGRESS_ALLOW_CIDR` | `--egress-allow-cidr` | _(none)_ | _(controller)_ Comma-separated CIDRs the controller may connect to, beyond the public internet, when fetching a tenant-authored `JwtAuth.spec.jwks.remote.uri`. Empty refuses every reserved/special-purpose destination (RFC 1918, loopback, link-local/cloud-metadata, CGNAT, documentation ranges, multicast) so a tenant can't turn the controller into an SSRF proxy; list a CIDR (e.g. the cluster's Service CIDR) to permit an in-cluster identity provider. A listed CIDR is also the only destination class where a plaintext `http://` `remote.uri` is accepted — everything else requires `https://`. See [JwtAuth](../gateway-api/route-extensions.md#jwt-authentication) |
