...
| `coxswain_discovery_streams_total` | Server | Counter | `result` (`accepted`/`rejected`/`rejected_not_leader`) — `rejected` covers wire-version mismatch, malformed scope, SVID/scope-binding denial, and a `node_id` already claimed by a **different** authenticated identity (a hijack attempt, or two nodes that ended up with the same `node_id` — a persistent condition, retried with ordinary exponential backoff client-side, not a fast retry); `rejected_not_leader` is a standby replica declining the stream |
| `coxswain_discovery_acks_total` | Server | Counter | — (snapshot Acks received; tracks fleet convergence throughput) |
| `coxswain_discovery_roster_reports_total` | Server | Counter | `result` (`accepted`/`partial`/`rejected`) — `RosterReport` fold outcomes. `accepted`: an authorized relay's report, every child in its own scope — the expected steady state. `partial`: an authorized relay named a child outside its own scope (another tenant's Gateway, or the shared pool); a well-formed relay can never produce this, so it should be flat at zero forever — alert on it. `rejected`: the sender was not authorized at all (a plain proxy, unauthenticated connection, or a relay whose namespace just started terminating, racing its own teardown) — not always zero, but a sustained or climbing rate outside of relay/namespace teardown is the signal worth alerting on |
...
When you run a [dedicated proxy pool](../gateway-api/index.md#dedicated-proxy-pools), you usually want to slice its metrics by Gateway. Coxswain does **not** bake the Gateway identity into the emitted series — a `gateway_name` / `gateway_namespace` label on every counter would multiply request-counter cardinality across every dedicated Gateway, and the shared pool has no single Gateway to name.
...
on the winning Ingress. See [Ingress annotation reference](../ingress/annotations.md)
