...
| `coxswain_discovery_streams_total` | Server | Counter | `result` (`accepted`/`rejected`/`rejected_not_leader`) — `rejected` covers wire-version mismatch, malformed scope, SVID/scope-binding denial, and a `node_id` colliding with an already-connected relay (a hijack attempt, or a legitimate relay losing its own reconnect race); `rejected_not_leader` is a standby replica declining the stream |
