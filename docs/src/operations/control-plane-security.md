...
A `node_id` is also unauthenticated — a stream's own choice, not derived from its SVID — so a second connect claiming a live `node_id` is refused rather than silently displacing it, for **every** stream, not just relays: the guard binds each `node_id` to the connecting peer's identity fingerprint, and grants only that same identity's own reconnect — never a different one. Without it, any authenticated peer could reclaim another node's row and, on its own immediate disconnect, evict its subtree — for a relay specifically, that means the real relay's entire folded set of leaves, without ever sending a `RosterReport`. Because a node's own reconnect is always granted, a rejection can only mean a genuinely different identity currently holds the `node_id` — a persistent misconfiguration (most plausibly two nodes that ended up with the same name; Kubernetes only guarantees Pod names unique within one namespace, not cluster-wide), not a transient race, so it is retried with ordinary exponential backoff rather than fast — retrying fast would just hammer the controller while the collision remains unresolved. The discovery server also actively pings idle connections and closes ones that stop answering, so a node's own stale session (a conntrack eviction, an asymmetric network partition) is reaped promptly rather than leaving its row and any folded subtree stuck to a dead connection.

The roster rejection path has a live hostile-client proof: a pod running the real `serve relay --shared` binary under an ordinary ServiceAccount — bootstrap authenticates any ServiceAccount that passes `TokenReview`, with no identity allowlist — bootstraps successfully and streams, but its `RosterReport` is silently dropped, since enforcing the shared-relay identity is the server's job, not a property of which binary the client happens to run.
