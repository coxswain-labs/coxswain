...
Each proxy also reports its **actually-bound listener ports** back over the same stream (a `NodeStatus` message, sent on stream open and on every bind change). The leader keys the Gateway `Programmed` condition on these reports: a shared-mode Gateway flips `Programmed=True` only once **every** connected shared-pool proxy has bound the Gateway's VIP ports (its per-Gateway internal `targetPort`s), and a dedicated Gateway only once its own proxy has bound the effective listener ports. Until then `Programmed` holds at `False/Pending` with its `observedGeneration` one below the current generation, and the condition message names what is being waited on. Once a generation is `Programmed=True`, pool churn (rollouts, leader failover) never flaps it back — only a spec change re-arms the gate.
