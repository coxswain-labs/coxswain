...
5. After each work cycle: `cargo fmt && cargo test --workspace --exclude coxswain-e2e`. Then ask the user what's next via `AskUserQuestion` (two questions in one call):
   ...
   - **Q2 (header "E2E suite")**: a by-plane suite (routing / tls / traffic_policy / status_conditions / provisioning_rbac / resilience / observability) / conformance / N/A. Consulted only when Q1 = "Run e2e tests".
