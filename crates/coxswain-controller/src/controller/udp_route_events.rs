//! Kubernetes API calls that write `UDPRoute` status patches.
//!
//! Sibling of `tcp_route_events.rs` — forked for the `UDPRoute` concrete type per the
//! no-generic-reconciler constraint in issue #33. UDPRoute has no `hostnames` field and
...
use coxswain_reflector::gw_types::{
    UdpRoute,
    v::udproutes::{UdpRouteParentRefs, UdpRouteStatusParents, UdpRouteStatusParentsParentRef},
};
...
/// Write `Accepted`, `Programmed`, and `ResolvedRefs` conditions on a `UDPRoute`
...
pub(super) async fn mark_udp_route_programmed(
    client: &Client,
    route: &UdpRoute,
    controller_name: &str,
    ...
    let owned_refs = filter_owned_udp_parent_refs(parent_refs, ns, owned_gateways);
    if owned_refs.is_empty() {
        tracing::debug!(name, ns, "Skipping status patch — no owned parentRefs");
        return;
    }

    let api: Api<UdpRoute> = Api::namespaced(client.clone(), ns);
    let now = Time(k8s_openapi::jiff::Timestamp::now());
    let Some(observed_gen) = route.metadata.generation else {
        tracing::warn!(
            name,
            ns,
            "Skipping UdpRoute status patch: metadata.generation is unset"
        );
        return;
    };

    let default_status = RouteParentStatus::default();
    let parents: Vec<UdpRouteStatusParents> = owned_refs
        .iter()
        .map(|p| {
    ...
    if route_status_unchanged(
        ...
        tracing::debug!(name, ns, "UdpRoute status already current — skipping patch");
        return;
    }
    ...
    let result = api
        ...
        .await;
    crate::metrics::observe_status_patch("udp_route", started, &result);
    match result {
        Ok(_) => tracing::info!(name, ns, "UdpRoute programmed"),
        Err(e) => tracing::warn!(name, ns, error = %e, "Failed to patch UdpRoute status"),
    }
}

fn filter_owned_udp_parent_refs(
    parent_refs: &[UdpRouteParentRefs],
    default_ns: &str,
    owned_gateways: &HashSet<ObjectKey>,
) -> Vec<UdpRouteParentRefs> {
    parent_refs
        .iter()
        .filter(|p| {
...
fn route_status_unchanged(
    desired: &[UdpRouteStatusParents],
    existing: Option<&[UdpRouteStatusParents]>,
) -> bool {
...
fn parent_ref_eq(a: &UdpRouteStatusParentsParentRef, b: &UdpRouteStatusParentsParentRef) -> bool {
    a.name == b.name
