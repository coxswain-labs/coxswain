//! [`RouteLike`] impl for `UDPRoute` — the UDPRoute-specific projections.
...
impl RouteLike for UdpRoute {
    ...
    fn route_hostnames(&self) -> Vec<&str> {
        // UDPRoute has no `hostnames` field — routing is by listener port only.
        Vec::new()
    }
