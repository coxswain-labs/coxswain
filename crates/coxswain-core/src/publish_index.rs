...
/// `generation` was first included in the published routing cells.
#[non_exhaustive]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PublishStamp {
    /// `metadata.generation` of the Gateway as of the stamping rebuild.
    pub generation: i64,
    /// Rebuild sequence assigned when this generation was first published.
    /// Stable across subsequent rebuilds at the same generation — the gate's
    /// target must not move under unrelated churn.
    pub seq: u64,
}
...
impl SharedGatewayPublishIndex {
    ...
    /// `(gateway, generation)` pairs, and return the rebuild's sequence.
    ///
    ...
    /// server's capture relies on. Stamps are sticky per generation: a
    /// Gateway whose generation is unchanged keeps its existing (older)
    /// sequence so the ack gate's target stays fixed under unrelated churn;
    /// a new generation gets this rebuild's sequence. Gateways absent from
    /// `published` drop out of the index (deleted or no longer owned).
    pub fn stamp_rebuild(&self, published: impl IntoIterator<Item = (ObjectKey, i64)>) -> u64 {
        ...
        for (key, generation) in published {
            let stamp = match prior.get(&key) {
                Some(existing) if existing.generation == generation => *existing,
                _ => PublishStamp { generation, seq },
            };
...
mod tests {
    ...
    fn first_stamp_uses_the_rebuild_seq() {
        ...
        let seq = idx.stamp_rebuild([(key("gw"), 1)]);
        assert_eq!(seq, 1);
        assert_eq!(
            idx.get(&key("gw")),
            Some(PublishStamp {
    ...
    fn absent_gateway_drops_out_of_the_index() {
        let idx = SharedGatewayPublishIndex::new();
        idx.stamp_rebuild([(key("gw"), 1), (key("other"), 4)]);
        idx.stamp_rebuild([(key("other"), 4)]);
