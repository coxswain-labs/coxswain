...
impl GeneratedCert {
    ...
    /// Generate a self-signed certificate valid for `host` with an additional URI SAN.
    ///
    /// The resulting cert carries two SANs: a DNS SAN for `host` and a URI SAN for
    /// `uri_san` (e.g. a SPIFFE ID like `spiffe://cluster.local/ns/default/sa/echo`).
    /// Used for `BackendTLSPolicy.spec.validation.subjectAltNames` (GEP-1897 #133) e2e tests.
    pub fn for_host_with_uri_san(host: &str, uri_san: &str) -> Self {
        let mut params = CertificateParams::new(vec![host.to_string()])
            .unwrap_or_else(|e| panic!("rcgen params for {host}: {e}"));
        params.subject_alt_names.push(SanType::URI(
            uri_san
                .try_into()
                .unwrap_or_else(|e| panic!("rcgen URI SAN for {uri_san}: {e}")),
        ));
        let key = KeyPair::generate().unwrap_or_else(|e| panic!("rcgen key for {host}: {e}"));
        let cert = params
            .self_signed(&key)
            .unwrap_or_else(|e| panic!("rcgen self-signed cert for {host} + URI SAN: {e}"));
        Self {
            cert_pem: cert.pem(),
